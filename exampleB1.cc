...
//	G4int SourceSelect=2;
 ...
 if ( argc >1 ) {
	...
	/*
	 if (FilterFlag) {
	 std::ofstream primFile("PrimariesX" + std::to_string((G4int)x0Scan) + "_Z" + std::to_string((G4int)ZValue) + "_CuD" + std::to_string((G4int)CuDiam) + "Fil.dat", std::ios::out);
	 } else {*/
 ...
 //  G4cout<<"PROVA MAIN 1 "<<x0Scan<<G4endl;
	...
	//
	//  G4cout<<"PROVA MAIN 2 "<<x0Scan<<G4endl;
