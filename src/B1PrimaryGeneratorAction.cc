...

{
	...
	if (fSourceSelect == 6)
		FlatEle = true;
	if (fSourceSelect == 7 || fSourceSelect == 511)
		FlatGamma = true;

	if (fExtSourceFile != "")
		hepmcAscii = new HepMCG4AsciiReader(fExtSourceFile); // path must be relative to where the code runs (eg build directory)
	else
	{
...
void B1PrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent)
{
	...
	else if (fSourceSelect == 6 || fSourceSelect == -1)
	{ // Flat Ele
		FlatEle = true;
	}
	else if (fSourceSelect == 7 || fSourceSelect == 511)
	{ // Flat Gamma (or 511keV)
		FlatGamma = true;
	}
	else if (fSourceSelect == 8)
	{ // Cu67 volume source
		Z = 29;
		A = 67;
	}
	else if (fSourceSelect == 9 || fSourceSelect == 11)
	{ // F18 volume source
		Z = 9;
		A = 18;
	}
	else if (fSourceSelect == 13)
	{ // Pointlike Ba137
		Z = 56;
		A = 133;
	}
	else if (fSourceSelect < -1)
	{ // Variable Radioactive Isotope source: -Source -ZZAA
	...
	if (fSourceSelect == 1 || fSourceSelect == 2 || fSourceSelect == 6)
	{ // se ho una delle due pasticche di Sr ignoro il TBR e faccio la pasticca di sorgente
		fRadiusMax = fRadiusInt;
		fRadiusMin = 0 * mm;
		zSource = -zSourceOffset;
	}
	else if (fSourceSelect == 3)
	{ // Extended source with TBR
		G4double random = G4UniformRand();
		if (random <= ProbA)
		{ // faccio il cilindretto cavo esterno al centro (VolA)
			fRadiusMax = fRadiusExt;
			fRadiusMin = fRadiusInt;
			fZ = fDZExt;
			zSource = -G4UniformRand() * fZ - zSourceOffset;
		}
		else if (random > ProbA && random <= ProbA + ProbB)
		{ // faccio il cilindretto attivo al centro (VolB) SEGNALE!!!!
			fRadiusMax = fRadiusInt;
			fRadiusMin = 0 * mm;
			fZ = fDZInt;
			zSource = -G4UniformRand() * fZ - zSourceOffset;
		...
		{ // faccio il cilindretto dietro a quello attivo al centro (VolC)
			fRadiusMax = fRadiusInt;
			fRadiusMin = 0 * mm;
			fZ = fDZExt - fDZInt;
	...
	else if (fSourceSelect == 11 && fGaSet == 3)
	{
		...
		if (random < volFuori / (volFuori + volDietro))
		{ // bordo
	...
	// ##########################
	if (FlatEle)
	{ // If FlatEle source (for Eff) was requested, generate only towards up
	...
	if (fSourceSelect == 10 && fExtSourceFile != "")
	{ // se ho richiesto la sorgente da file esterno, e c'è un file
