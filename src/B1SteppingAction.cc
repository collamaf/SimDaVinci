...
#pragma mark Annihilation
	...
	if ((fSourceChoice == 4 || fSourceChoice == 11) && step->GetTrack()->GetDynamicParticle()->GetPDGcode() == -11 && step->GetPostStepPoint()->GetProcessDefinedStep() && step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName() == "annihil")
	{
...
#pragma mark Entering Pter
	...
	if ((NextVol && ThisVol->GetName() != "Pter" && NextVol->GetName() == "Pter"))
	{ // what enters Pter (form every different volume)
		...
		if (debug)
			G4cout << " SETTO fEventAction->GetEnteringParticle()= " << fEventAction->GetEnteringParticle() << G4endl << G4endl;

		// Check if the current track had already enetered somehow the PTER (to avoid double counting), otherwise increase the counter
		if (fEventAction->GetPterStoreTrackID() == step->GetTrack()->GetTrackID())
		{										 // if I already saw this track exiting the source...
			fEventAction->AddPterPassCounter(1); // increase the counter
			if (debug)
				G4cout << "STEPDEBUGPTER Traccia gia vista! " << fEventAction->GetPterStoreTrackID() << ", PassCounter= " << fEventAction->GetPterPassCounter() << G4endl;
		}
		else
		{
			fEventAction->SetPterStoreTrackID(step->GetTrack()->GetTrackID());
			if (debug)
				G4cout << "STEPDEBUGPTER PterDEBUG PRIMO PASSAGGIO!! della traccia " << fEventAction->GetPterStoreTrackID() << ", PassCounter= " << fEventAction->GetPterPassCounter() << G4endl;
		}
...
#pragma mark Exiting Source
	...
	if (fSourceChoice != 5 && fSourceChoice != 7 && NextVol && ThisVol->GetName() != "DummyExitSorg" && NextVol->GetName() == "DummyExitSorg" && step->GetPreStepPoint()->GetMomentumDirection().z() > 0) // New (2019.01.16) logic: if I go from somewhere into DummyExitSorg with a negative Z direction..
	{
		...
		if (fEventAction->GetSourceExitStoreTrackID() == step->GetTrack()->GetTrackID())
		{											   // if I already saw this track exiting the source...
...
#pragma mark Exiting Absorber
	...
	if (fAbsHoleDiam >= 0 && NextVol && ThisVol->GetName() == "Absorber" && NextVol->GetName() == "DummyExitAbs")
	{
		...
		if (fEventAction->GetPostAbsStoreTrackID() == step->GetTrack()->GetTrackID())
		{											// if I already saw this track exiting the absorber...
...
#pragma mark Entering Probe
	...
	if (NextVol && ThisVol->GetName() == "DummyEnterProbe" && NextVol->GetName().contains("FrontShield"))
	{
		...
		if (fEventAction->GetPreProbeStoreTrackID() == step->GetTrack()->GetTrackID())
		{											 // if I already saw this track exiting the absorber...
...
#pragma mark Inside Pter
	...
	if (ThisVol->GetLogicalVolume() == fScoringVolume && step->GetTrack()->GetDynamicParticle()->GetPDGcode() != 0)
	{
		...
		if (0 && debug)
			G4cout << "CIAODEBUG Ho un rilascio di energia (" << step->GetTotalEnergyDeposit() / keV << " [KeV]) dovuto ad una particella entrata nel CMOS di tipo: " << fEventAction->GetEnteringParticle() << G4endl;

		if (fEventAction->GetEnteringParticle() == 11)
		{ // if son of electron
			fEventAction->AddEdepEle(step->GetTotalEnergyDeposit());
		}
		else if (fEventAction->GetEnteringParticle() == -11)
		{ // if son of positron
			fEventAction->AddEdepPos(step->GetTotalEnergyDeposit());
		}
		else if (fEventAction->GetEnteringParticle() == 22)
		{ // if son of photon
			fEventAction->AddEdepFot(step->GetTotalEnergyDeposit());
			if (debug && step->GetTotalEnergyDeposit() > 0)
				G4cout << "CONTROLLA" << G4endl;
		}
