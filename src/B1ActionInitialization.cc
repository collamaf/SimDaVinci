...
B1ActionInitialization::B1ActionInitialization(G4double x0, G4double ZValue, G4double CuDiam, G4int FilterFlag, std::ofstream& file, G4double TBR/*, G4bool SrSourceFlag*/, G4int SourceSelect)
  : G4VUserActionInitialization(), fX0Scan(x0), fZValue(ZValue), fCuDiam(CuDiam), fFilterFlag(FilterFlag), FilePrimaries(file), fTBR(TBR), /*fSrSourceFlag(SrSourceFlag),*/ 	fSourceSelect(SourceSelect)
{}
...
{
  B1RunAction* runAction = new B1RunAction(fX0Scan, fZValue, fCuDiam, fFilterFlag, fTBR, fSourceSelect);
